...
class InventoryStats:
    def __init__(self):
        ...
        self.filled_from_snowflake = []   # (model, column)
...
def build_inventory(config, rows, headers, snowflake_type_lookup=None):
    ...
    snowflake_type_lookup: optional callable (model_name, column_name) -> raw_type_or_"".
    Third-tier fallback, tried only when the mapped column and data_type_fallback are
    both blank. Flows through the same type_normalization as the other two tiers.
    """
    ...
    for row in rows:
        ...
        # data_type: mapped column, optional same-row fallback, optional live
        # Snowflake lookup, then normalize.
        raw_type = _s(row.get(m["data_type"]))
        if not raw_type and fallback:
            raw_type = _s(row.get(fallback))
        if not raw_type and snowflake_type_lookup:
            raw_type = _s(snowflake_type_lookup(model_name, column_name))
            if raw_type:
                stats.filled_from_snowflake.append((model_name, column_name))
...
def load_inventory(config, source_name, excel_path=None, snowflake_type_lookup=None):
    ...
    return build_inventory(config, rows, headers, snowflake_type_lookup=snowflake_type_lookup)
